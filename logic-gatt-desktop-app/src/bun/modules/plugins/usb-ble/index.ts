...
class UsbBlePlugin extends PluginBase {
  ...
  private async startPythonBackend(): Promise<void> {
    ...
    try {
      ...
      if (!backend) {
        throw new Error('Bluetooth adapter support is not installed in this build.')
      }
      ...
      if (!this.pythonProcess) {
        throw new Error('The Bluetooth adapter could not start — another instance of the app may already be using it.')
      }
      await this.connectToPythonWs()
    } finally {
