...
export function ConnectionPanel() {
  ...
  useEffect(() => {
    const push = (m: string) => setLog(l => [`${new Date().toLocaleTimeString()}  ${m}`, ...l].slice(0, 8))
    ...
    <div style={card}>
      ...
      {awaiting && (
        <div style={pendingCard}>
          ...
          <div style={{ color: '#d1d5db', marginBottom: 8 }}>
            {awaiting.address} connected without scanning the QR code. Allow it only if it is your phone.
          </div>
      ...
      {info ? (
        <>
          ...
          <div style={{ textAlign: 'center', marginBottom: 8, wordBreak: 'break-all' }}>{info.displayUrl}</div>
          <div style={{ color: '#9ca3af', marginBottom: 6 }}>
            Scanning the QR connects straight away. Auto-discovery via mDNS (<code>_logicgatt._tcp</code>) also works,
            but asks for approval here first.
          </div>
        </>
      ) : (
