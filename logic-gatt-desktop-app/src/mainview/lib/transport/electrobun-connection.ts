...
import type { PluginEvent, Schema as WireSchema, DeviceSettings as WireDeviceSettings } from '@logic-gatt/shared'
  ...
  return {
    services: schema.map(svc => ({
      ...
      characteristics: svc.characteristics.map(chr => ({
        ...
        defaultValue: chr.defaultValue ? Array.from(parseHex(chr.defaultValue)) : undefined,
      })),
    })),
  }
}
...
function toWireSettings(settings: DeviceSettings): WireDeviceSettings {
  return {
    ...
    manufacturerData: settings.manufacturerData ? Array.from(parseHex(settings.manufacturerData)) : [],
    serviceUuids16Bit: [],
  }
}
