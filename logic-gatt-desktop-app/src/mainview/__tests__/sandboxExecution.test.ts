/**
 * Tests for `runSandboxed` end to end (`lib/sandbox.ts`).
 *
 * The level above `sandbox.test.ts`, which covers the codecs and the binary
 * reader/writer as units: here a whole user function body is executed and the result,
 * the logs, and everything `ctx` exposes are checked from the outside.
 */
